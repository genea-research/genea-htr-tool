...
"""
    ...
    def create_individual_pdf(self, image_path: str, transcription: str, output_filename: str) -> str:
        ...
        # Create PDF folder in the same directory as the source image
    ...
    def create_individual_txt(self, image_path: str, transcription: str, output_filename: str) -> str:
        ...
        try:
            ...
            header += f"Generated by {self.provider_name.upper()}: {self.transcription_config['primary']['model']}\n"
    ...
    def create_csv_file(self, results: List[Dict], source_directory: str) -> str:
        ...
        try:
            with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                current_date = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                model_name = f"{self.provider_name.upper()}: {self.transcription_config['primary']['model']}"
        ...
        try:
            ...
            if self.output_format == "PDF":
                if is_pdf_page:
                    ...
                    output_filename = f"{original_name}_page_{pdf_page_num}.pdf"
                else:
                    # For regular images, use original filename
                    output_filename = f"{Path(original_file).stem}.pdf"
                
                # Create the PDF using the processed image
                output_path = self.create_individual_pdf(image_path, transcription, output_filename)
            elif self.output_format == "TXT":
                if is_pdf_page:
                    # For PDF pages, include page info in filename
                    original_name = Path(original_file).stem
                    pdf_page_num = image_info['page_number']
                    output_filename = f"{original_name}_page_{pdf_page_num}.txt"
                else:
                    # For regular images, use original filename
                    output_filename = f"{Path(original_file).stem}.txt"
                
                # Create the TXT using the processed image
                output_path = self.create_individual_txt(image_path, transcription, output_filename)
            elif self.output_format == "CSV":
                # For CSV, we don't create individual files - the CSV will be created at the end
                self.logger.info(f"Collected transcription for CSV: {display_name}")
    ...
    def process_batch(self, input_dir: str, output_filename: str = "transcribed_documents.pdf") -> Dict:
            ...
            if self.max_workers == 1:
                ...
                for i, image_info in enumerate(image_files, 1):
                    ...
                    if result["output_path"]:
                        pdf_paths.append(result["output_path"])
            else:
        ...
        if self.output_format.upper() == "CSV" and results:
            try:
                ...
                csv_path = self.create_csv_file(results, input_dir)
